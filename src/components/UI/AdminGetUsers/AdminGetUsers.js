...
const AdminGetUsers = () => {
  ...
  const getAdminProfile = async () => {
    try {
      if (!token) {
        ...
        console.log(response);
        if (response.status === 200) {
          if (response.data.rows == null || response.data.rows === undefined) {
            setAdminProfile([]);
          } else {
  ...
  return (
    <div className="admin-wrapper">
      {hasNoToken && <p style={{ color: "red" }}>{tokenErrorMsg}</p>}
      {showCaughtError && <p>sorry something went wrong</p>}
      {/* Admin profile*/}
      {/* <div className="admin-profile">
        {adminProfile.map((adminInfo) => {
          return (
            <div key={adminInfo.userid}>
              <p>Admin firstname: {adminInfo.firstname}</p>
              <p>Admin lastname: {adminInfo.lastname}</p>
